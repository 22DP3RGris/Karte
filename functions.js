// Gaida, kad dokuments ir pilnībā ielādēts.
document.addEventListener("DOMContentLoaded", (event) => {
    // Izgūst datus no geomap.json
    fetch('geomap.json')
        .then(response => response.json())
        // Ja viss kārtībā, tad izsauc kartes inicializācijas funkciju.
        .then(geojson => { initializeMap(geojson)})
        .catch(error => console.error("Kļūda ielādējot JSON failu:", error));
});

var LKS92WGS84 = (function()
{
    // Koordinātu parveidojumos izmantotās konstantes
...
// Kartes inicializācijas funkcija.
function initializeMap(geojson){
    // Izviedo karti un novieto to sākotnējā pozīcījā
    var map = L.map('map').setView([56.95, 24.1], 7); // Nocentrē Latvijā.

    // Izveido kartes slāni, nosaka minimālo un maksimālo pietuvināšanas sliekšņus.
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        maxZoom: 18,
        minZoom: 8,
    }).addTo(map);

    // Izveido jaunu ikonu priekš marķieriem.
    var myIcon = L.icon({iconUrl: 'img/marker.png',
                         iconSize: [20, 30]
    })

    // Cikls, kas iet cauri pilnīgi visiem Features objektiem.
    geojson.features.forEach(function(feature) {
        // Nosaka konkrētā Feature koordinātes.
        var coords = feature.geometry.coordinates;
        // Pārvērš koodrinātes nepieciešamajā formātā. dokumentācijā: marker(<latLng>)
        var latLng = LKS92WGS84.convertXYToLatLon(coords);
        // Noteiktajās koodrinātēs izvieto iepriekš sagatavoto marķieri un pievieno pop-up lodziņu, kas glabās informāciju par Feature nosaukumu.
        L.marker(latLng, {icon: myIcon}).addTo(map).bindPopup("<b>" + feature.properties.PLACENAME + "</b>")
    });
}
